                        ...
                        <form method="post" action="{{ route('owner.shops.update', [ 'shop' => $shop->id ])}}" enctype="multipart/form-data">
                           @csrf
                                <div class="-m-2">
                                    ...
                                    <div class="p-2 w-1/2 mx-auto">
                                        <div class="relative">
                                            <label for="name" class="leading-7 text-sm text-gray-600">画像</label>
                                    ...
                                    <div class="p-2 w-1/2 mx-auto">
                                        <div class="relative flex justify-around">
                                            <div>
                                                <input type="radio" id="image" name="is_selling" class="mr-2" value="1" @if($shop->is_selling === 1){ checked } @endif>販売中
                                            </div>
                                            <div>
                                                <input type="radio" id="image" name="is_selling" class="mr-2" value="0" @if($shop->is_selling === 0){ checked } @endif>停止中
                                            </div>
                                        </div>
                                    </div>
                                    <div class="p-2 mt-4 w-full flex justify-around">
                                        <button type="button" onclick="location.href='{{ route('owner.shops.index') }}'" class="text-white bg-gray-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">戻る</button>
